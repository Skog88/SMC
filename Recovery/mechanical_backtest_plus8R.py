...
--- RECOVERY NOTE ---
This is the snapshot that produced GBPUSD_no_buffer_plus8R_trades.csv (40 trades,
12W/28L, +8.0R).

Key difference from the corrected version:
  Line 81: end_time = end_time or datetime.now()   ← machine local time (UTC+2)
  (corrected code uses datetime.utcnow())

Additionally, at the time this run was made, C:/mt5-mcp/mt5_mcp_server.py used:
...
Combined effect: all timestamps were UTC+5 (broker UTC+3 + machine UTC+2).
The backtest window comparison (start_time <= c.time <= end_time) was internally
consistent, so trade outcomes are valid — but bar times shown in the CSV are
5 hours ahead of true UTC and must NOT be used for chart alignment.
"""
...
# Point exclusively at the recovery MCP server (old fromtimestamp behaviour).
# The live server at C:/mt5-mcp/mt5_mcp_server.py is NOT touched.
_OLD_MCP_CONFIG = Mt5McpConfig(
    server_script=Path(__file__).resolve().parent / "mt5_mcp_server_old.py"
)
...
def run_symbol_backtest(
    ...
    engine = (
        LocalDataEngine(data_dir) if data_dir is not None
        else Mt5DataEngine(client=Mt5McpClient(_OLD_MCP_CONFIG))
    )
    ...
    end_time = end_time or datetime.now()  # NOTE: machine local time — see recovery note above
